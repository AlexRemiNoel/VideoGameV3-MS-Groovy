...
import org.springframework.test.context.ActiveProfiles; // Import if using profiles
...
class UserRepositoryIntegrationTest {
    ...
    void setUp() {
        ...
        user1 = new User(userIdObject1, "repoUser1", "repo1@test.com", "pass1", 100.0, orders1, games1);
        user2 = new User(userIdObject2, "repoUser2", "repo2@test.com", "pass2", 50.0, orders2, games2);
    }
    ...
    void findUserByUserId_uuid_whenUserExists_thenReturnUser() {
        ...
        assertEquals(user1.getOrders(), foundUser.getOrders());
    ...
    void save_whenNewUser_thenPersistUser() {
        ...
        assertThat(savedUser.getOrders()).containsExactlyInAnyOrderElementsOf(user1.getOrders()); // Verify collections
        assertThat(savedUser.getGames()).containsExactlyInAnyOrderElementsOf(user1.getGames());

        assertNotNull(foundUser);
        assertEquals(user1.getEmail(), foundUser.getEmail());
        assertEquals(user1.getUserId(), foundUser.getUserId());
        assertThat(foundUser.getOrders()).containsExactlyInAnyOrderElementsOf(user1.getOrders());
    ...
    @Test
    void findAll_whenMultipleUsersExist_thenReturnAllUsers() {
        // Arrange
        userRepository.save(user1);
